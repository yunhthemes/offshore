								...
								@if($company_types->count() > 0)
					              	@foreach($company_types as $k => $company_type)
					                    <tr id="id-{{$company_type->id}}" class="company-type-id tbl-item">
					                    	...
					                    	<td class="name"><a class="link-to" href="{{ route('admin.jurisdiction.show', $company_type->id) }}">{{ $company_type->name }}</a></td>			
					                    	<td><a href="{{ route('admin.jurisdiction.edit', $company_type->id) }}"><button class="custom-submit-class custom-submit-class-2">Edit</button></a></td>
					                    </tr> 
					              	@endforeach
					            @else					
		...
		$(document).ready(function(){
			$("#remove-selected").on("click", function(e){
				...
				if(IdsToDelete.length > 0) {
					$.ajax({
					    url: "{{ url('admin/jurisdiction') }}/"+IdsToDelete,
