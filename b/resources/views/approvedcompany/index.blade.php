								...
								@if($companies->count() > 0)							
					              	@foreach($companies as $k => $company)
					                    <tr id="id-{{$company->id}}" class="company-id list-item box">
					                    	...
					                    	<td><span class="date">{{ (strtotime($company->incorporation_date) < 0) ? $company->incorporation_date : date('d M Y', strtotime($company->incorporation_date)) }}</span></td>
					                    	<td class="username">
				                    	 	...
				                    	 	<td><a href="{{ route('admin.approvedcompany.show', $company->id) }}"><button class="custom-submit-class">View</button></a><a href="{{ route('admin.approvedcompany.edit', $company->id) }}"><button class="custom-submit-class">Edit</button></a></td>		                    						
					                    </tr> 
					              	@endforeach			              	
					            @else					
