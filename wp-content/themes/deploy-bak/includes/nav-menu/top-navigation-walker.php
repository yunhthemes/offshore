 ...
 */
if (!class_exists('DeployMikadoTopNavigationWalker')) {
	class DeployMikadoTopNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if($depth == 0){
				$out_div = '<div class="second '.$background_image_class.'" '.deploy_mikado_get_inline_style($menu_image_background).'><div class="inner">';
			}else{
				$out_div = '';
			}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->hide == ""){
				...
				if($args->has_children && $angle_icon != "") {
					$item_output .= '<i class="q_menu_arrow '.$angle_icon.'"></i>';
				}
