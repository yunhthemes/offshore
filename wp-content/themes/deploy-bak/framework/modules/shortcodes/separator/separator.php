<?php
namespace Deploy\MikadofModules\Separator;
...
class Separator implements ShortcodeInterface{
			...
			array(
				...
				'params' => array(
					...
					array(
						...
						'param_name' => 'color',
						'value' => '',
						'dependency' => array('element' => 'type', 'value' => array('normal', 'full_width'))
					),
					array(
						'type' => 'dropdown',
	...
	public function render($atts, $content = null) {
		...
		$html = deploy_mikado_get_shortcode_module_template_part('templates/separator-template', 'separator', '', $params);
	...
	private function getSeparatorStyle($params) {
		...
		if ($params['width'] !== '') {
			if(deploy_mikado_string_ends_with($params['width'], '%') || deploy_mikado_string_ends_with($params['width'], 'px')) {
		...
		if ($params['top_margin'] !== '') {
			if(deploy_mikado_string_ends_with($params['top_margin'], '%') || deploy_mikado_string_ends_with($params['top_margin'], 'px')) {
		...
		if ($params['bottom_margin'] !== '') {
			if(deploy_mikado_string_ends_with($params['bottom_margin'], '%') || deploy_mikado_string_ends_with($params['bottom_margin'], 'px')) {
