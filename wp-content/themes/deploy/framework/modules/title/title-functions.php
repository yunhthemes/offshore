...
if(!function_exists('deploy_mikado_get_title')) {
	/**
	 * Loads title area HTML
	 */
	function deploy_mikado_get_title() {
		$id = deploy_mikado_get_page_id();

		extract(deploy_mikado_title_area_height());
		extract(deploy_mikado_title_area_background());
		...
		if(get_post_meta($id, "mkdf_show_title_area_meta", true) !== "") {
			...
			$show_title_area = deploy_mikado_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
		}
		...
		if(get_post_meta($id, "mkdf_title_area_type_meta", true) !== "") {
			...
			$type = deploy_mikado_options()->getOptionValue('title_area_type');
		}
		...
		if(get_post_meta($id, "mkdf_title_area_enable_breadcrumbs_meta", true) !== "") {
			...
			$enable_breadcrumbs = deploy_mikado_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
		}
		...
		$show_title_text_separator = get_post_meta($id, 'mkdf_enable_title_text_separator_meta', true);
		$show_title_text_separator = $type == 'standard' && $show_title_text_separator === 'yes';
		...
		$parameters = array(
			...
			'show_title_text_separator'     => $show_title_text_separator
		);

		$parameters = apply_filters('deploy_mikado_title_area_height_params', $parameters);

		deploy_mikado_get_module_template_part('templates/title', 'title', '', $parameters);
	}
}

if(!function_exists('deploy_mikado_get_title_text')) {
	/**
	 * Function that returns current page title text. Defines deploy_mikado_title_text filter
	 ...
	 */
	function deploy_mikado_get_title_text() {
		...
		elseif(is_author()) {
			...
			$title = esc_html__('Author:', 'deploy')." ".get_the_author();
		...
		elseif(is_search()) {
			...
			$title = esc_html__('Search', 'deploy');
		} //is current page 404?
		elseif(is_404()) {
			//is 404 title text set in theme options?
			if(deploy_mikado_options()->getOptionValue('404_title') != "") {
				//get it from options
				$title = deploy_mikado_options()->getOptionValue('404_title');
			} else {
				//get default 404 page title
				$title = esc_html__('404 - Page not found', 'deploy');
			}
		} //is WooCommerce installed and is shop or single product page?
		elseif(deploy_mikado_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
		...
		elseif(deploy_mikado_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
			global $wp_query;
		...
		elseif(is_archive()) {
			$title = esc_html__('Archive', 'deploy');
		} //current page is regular page
		else {
			$title = get_the_title($id);
		}

		$title = apply_filters('deploy_mikado_title_text', $title);

		return $title;
	}
}

if(!function_exists('deploy_mikado_title_text')) {
	/**
	 * Function that echoes title text.
	 *
	 * @see deploy_mikado_get_title_text()
	 */
	function deploy_mikado_title_text() {
		echo deploy_mikado_get_title_text();
	}
}

if(!function_exists('deploy_mikado_subtitle_text')) {
	/**
	 * Function that echoes subtitle text.
	 *
	 */
	function deploy_mikado_subtitle_text() {
		$id       = deploy_mikado_get_page_id();
		$subtitle = '';
...
if(!function_exists('deploy_mikado_custom_breadcrumbs')) {
	/**
	 ...
	 * @see is_front_page()
	 * @see is_category()
	 * @see deploy_mikado_is_product_category()
	 * @see get_search_query()
	 */
	function deploy_mikado_custom_breadcrumbs() {
		...
		$delimiter   = '<span class="mkdf-delimiter" '.deploy_mikado_get_inline_style($bread_style).'>'.deploy_mikado_icon_collections()->renderIcon('arrow_right', 'font_elegant', array('icon_attributes' => array('class' => 'mkdf-delimiter-icon'))).'</span>'; // delimiter between crumbs
		...
		if(is_home() && !is_front_page()) {
			$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.deploy_mikado_get_inline_style($bread_style).' href="'.esc_url($homeLink).'">'.$home.'</a>'.$delimiter.' <a '.deploy_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.get_the_title($pageid).'</a></div></div>';

		} elseif(is_home()) {
			$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner">'.$before.$home.$after.'</div></div>';
		} elseif(is_front_page()) {
			if($showOnHome == 1) {
				$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.deploy_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a></div></div>';
			}
		} else {

			$output .= '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.deploy_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a>'.$delimiter;

			if(is_category() || deploy_mikado_is_product_category()) {
				...
				$output .= '<a '.deploy_mikado_get_inline_style($bread_style).' href="'.get_year_link(get_the_time('Y')).'">'.get_the_time('Y').'</a>'.$delimiter;
				$output .= '<a '.deploy_mikado_get_inline_style($bread_style).' href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('F').'</a>'.$delimiter;
				...
				$output .= '<a '.deploy_mikado_get_inline_style($bread_style).' href="'.get_permalink($parent).'">'.$parent->post_title.'</a>';
				if($showCurrent == 1) {
				...
				while($parent_id) {
					...
					$breadcrumbs[] = '<a '.deploy_mikado_get_inline_style($bread_style).' href="'.get_permalink($page->ID).'">'.get_the_title($page->ID).'</a>';
			...
			if(get_query_var('paged')) {
				...
				$output .= $before." (".esc_html__('Page', 'deploy').' '.get_query_var('paged').")".$after;
...
if(!function_exists('deploy_mikado_get_title_area_height')) {

	/**
	 * Function that returns title height
	 **/
	function deploy_mikado_get_title_area_height() {

		$id = deploy_mikado_get_page_id();

		$title_height = 93; // default title height without header height

		if(get_post_meta($id, "mkdf_title_area_height_meta", true) != '') {
			$title_height = get_post_meta($id, 'mkdf_title_area_height_meta', true);
		} elseif(deploy_mikado_options()->getOptionValue('title_area_height') !== '') {
			$title_height = deploy_mikado_options()->getOptionValue('title_area_height');
		}

		return apply_filters('deploy_mikado_title_area_height', $title_height);
	}
}

if(!function_exists('deploy_mikado_get_title_content_padding')) {
	/**
	 * Function that returns title content pading
	 **/

	function deploy_mikado_get_title_content_padding() {
		return apply_filters('deploy_mikado_title_content_padding', 0);
	}
}


if(!function_exists('deploy_mikado_title_area_height')) {
	/**
	 * Function that returns title height and padding to be applied in template
	 **/

	function deploy_mikado_title_area_height() {
		$id                            = deploy_mikado_get_page_id();
		$title_height_and_padding      = array();
		$title_height                  = deploy_mikado_get_title_area_height();
		$header_height_padding         = deploy_mikado_get_title_content_padding();
		...
		if(get_post_meta($id, "mkdf_title_area_vertial_alignment_meta", true) !== '') {
			...
			$title_vertical_alignment = deploy_mikado_options()->getOptionValue('title_area_vertial_alignment');;
		}
...
if(!function_exists('deploy_mikado_title_area_background')) {
	/**
	 * Function that returns title background style be applied in template
	 **/

	function deploy_mikado_title_area_background() {
		$id                           = deploy_mikado_get_page_id();
		$show_title_img               = true;
		...
		if(get_post_meta($id, "mkdf_title_area_background_color_meta", true) != "") {
			...
			$background_color = deploy_mikado_options()->getOptionValue('title_area_background_color');
		}
		...
		if(get_post_meta($id, "mkdf_title_area_background_image_meta", true) != "") {
			...
			$background_image = deploy_mikado_options()->getOptionValue('title_area_background_image');
		}

		//check for background image width
		$background_image_width = "";
		if($background_image !== '') {
			$background_image_width_dimensions_array = deploy_mikado_get_image_dimensions($background_image);
...
if(!function_exists('deploy_mikado_title_size_body_class')) {
	function deploy_mikado_title_size_body_class($classes) {
		$id                  = deploy_mikado_get_page_id();
	...
	add_action('body_class', 'deploy_mikado_title_size_body_class');
}