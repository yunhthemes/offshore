<?php
namespace Deploy\MikadofModules\SocialShare;
...
class SocialShare implements ShortcodeInterface {
	...
	public function render($atts, $content = null) {
		...
		$params['enable_social_share'] = (deploy_mikado_options()->getOptionValue('enable_social_share') == 'yes') ? true : false;

		//Is social share enabled for post type
		$post_type = get_post_type();
		$params['enabled'] = (deploy_mikado_options()->getOptionValue('enable_social_share_on_'.$post_type)) ? true : false;
		...
		if ($params['enable_social_share']) {
			if ($params['enabled']) {
				$html .= deploy_mikado_get_shortcode_module_template_part('templates/' . $params['type'], 'socialshare', '', $params);
			}
		}
	...
	private function getSocialNetworksParams($params) {
		...
		foreach ($this->socialNetworks as $net) {
			...
			$html = '';
			if (deploy_mikado_options()->getOptionValue('enable_'.$net.'_share') == 'yes') {
				...
				$params['custom_icon'] = (deploy_mikado_options()->getOptionValue($net.'_icon')) ? deploy_mikado_options()->getOptionValue($net.'_icon') : '';
				$html = deploy_mikado_get_shortcode_module_template_part('templates/parts/network', 'socialshare', '', $params);
	...
	private function getSocialNetworkShareLink($net, $image) {
		...
		switch ($net) {
			case 'facebook':
				$link = 'window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(deploy_mikado_addslashes(get_the_title())) . '&amp;p[summary]=' . urlencode(deploy_mikado_addslashes(get_the_excerpt())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=' . $image[0] . '\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');';
				break;
			case 'twitter':
				$count_char = (isset($_SERVER['https'])) ? 23 : 22;
				$twitter_via = (deploy_mikado_options()->getOptionValue('twitter_via') !== '') ? ' via ' . deploy_mikado_options()->getOptionValue('twitter_via') . ' ' : '';
				$link = 'window.open(\'http://twitter.com/home?status=' . urlencode(deploy_mikado_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
				break;
			case 'google_plus':
			...
			case 'tumblr':
				...
				break;
			case 'pinterest':
				$link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . deploy_mikado_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
				break;
			case 'vk':
