<?php
namespace Deploy\MikadofModules\CallToAction;
...
class CallToAction implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		$call_to_action_button_IconCollections = deploy_mikado_icon_collections()->iconCollections;
		...
		vc_map( array(
				...
				'params' => array_merge(
					...
					deploy_mikado_icon_collections()->getVCParamsArray(array('element' => 'type', 'value' => array('with-icon'))),
					array(
						array(
							'type' 			=> 'textfield',
						...
						array(
							...
							'value' => array_merge(array('No Icon' => ''),deploy_mikado_icon_collections()->getIconCollectionsVC()),
							'save_always' => true,
							'dependency' => array('element' => 'show_button', 'value' => array('yes'))
						)
					),
	...
	public function render($atts, $content = null) {
		...
		foreach (deploy_mikado_icon_collections()->iconCollections as $collection_key => $collection) {

			$call_to_action_icons_form_fields['button_' . $collection->param ] = '';

		}

		$args = array_merge($args, deploy_mikado_icon_collections()->getShortcodeParams(),$call_to_action_icons_form_fields);
		...
		$html = deploy_mikado_get_shortcode_module_template_part('templates/call-to-action-template', 'calltoaction', '', $params);
	...
	private function getCallToActionIcon($params) {
		...
		if(!empty($params[$icon])){			
			$call_to_action_icon = deploy_mikado_icon_collections()->renderIcon( $params[$icon], $params['icon_pack'], $iconStyles );
		}
	...
	private function getButtonParameters($params) {
		...
		if(!empty($params['button_icon_pack'])) {
			...
			$iconPackName = deploy_mikado_icon_collections()->getIconCollectionParamNameByKey($params['button_icon_pack']);
