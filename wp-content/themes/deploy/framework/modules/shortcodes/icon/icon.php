<?php
namespace Deploy\MikadofModules\Shortcodes\Icon;

use Deploy\MikadofModules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package Deploy\MikadofModules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
            ...
            'params'                    => array_merge(
                ...
                array(
                    ...
                    array(
                        ...
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'deploy'),
    ...
    public function render($atts, $content = null) {
        ...
        $html = deploy_mikado_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.deploy_mikado_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.deploy_mikado_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.deploy_mikado_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.deploy_mikado_filter_px($shapeSize).'px';
            }
            ...
            if(isset($params['border_width']) && $params['border_width'] !== '') {
                $iconHolderStyles[] = 'border-width: '.deploy_mikado_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.deploy_mikado_filter_px($params['border_radius']).'px';
                }
            }
        }
