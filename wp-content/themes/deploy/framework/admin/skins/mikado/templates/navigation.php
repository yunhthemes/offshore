<?php global $theme_name_php_global_Framework; ?>

<div class="mkdf-tabs-navigation-wrapper">
    <ul class="nav nav-tabs">
        <?php
        foreach (deploy_mikado_framework()->mkdOptions->adminPages as $key => $page ) {
            $slug = "";
            if (!empty($page->slug)) $slug = "_tab".$page->slug;
            ?>
            <li<?php if ($page->slug == $tab) echo " class=\"active\""; ?>>
                <a href="<?php echo esc_url(get_admin_url().'admin.php?page=deploy_mikado_theme_menu'.$slug); ?>">
                    <?php if($page->icon !== '') { ?>
        ...
        ?>
        <?php if (deploy_mikado_core_installed()) { ?>
        <li <?php if($isImportPage) { echo "class='active'"; } ?>><a href="<?php echo esc_url(get_admin_url().'admin.php?page=deploy_mikado_theme_menu_tabimport'); ?>"><i
