...
if( ! class_exists('DeployMikadoSidebar') ){
	
	class DeployMikadoSidebar{
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area', 'deploy');
		...
		function load_assets(){
			...
			wp_enqueue_script('mkdf-deploy-sidebar' , MIKADO_ROOT . '/framework/admin/assets/js/mkdf-sidebar.js');
			wp_enqueue_style( 'mkdf-deploy-sidebar' , MIKADO_ROOT . '/framework/admin/assets/css/mkdf-sidebar.css');
		}
		...
		function template_add_widget_field(){
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'deploy')."' name='mkd-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'deploy')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars(){
			...
			$args = apply_filters('deploy_mikado_custom_widget_args', $args);
