...
include_once 'mkdf-twitter-helper.php';

/**
 * Class MikadofTwitterApi
 */
class MikadofTwitterApi {
    /**
    ...
    const REQUEST_TOKEN_FIELD = 'mkdf_twitter_request_token';
    const REQUEST_TOKEN_SECRET_FIELD = 'mkdf_twitter_request_token_secret';
    const ACCESS_TOKEN_FIELD = 'mkdf_twitter_access_token';
    const ACCESS_TOKEN_SECRET_FIELD = 'mkdf_twitter_access_token_secret';
    const AUTHORIZE_TOKEN_FIELD = 'mkdf_twitter_authorize_token';
    const AUTHORIZE_VERIFIER_FIELD = 'mkdf_twitter_authorize_verifier';
    const USER_ID_FIELD = 'mkdf_twitter_user_id';
    const USER_SCREEN_NAME_FIELD = 'mkdf_twitter_screen_name';
    ...
    public function __construct() {
        ...
        $this->helper = new MikadofTwitterHelper();
    }
    /**
     ...
     *
     * @see MikadofTwitterApi::generateSignatureBase()
     */
    ...
    public function obtainRequestToken() {
        ...
        if(is_wp_error($response)) {
            ...
            $responseObj->message = __('Internal WP error', 'mkd_twitter_feed');
        } else {
            ...
            if(!empty($responseBody)) {
                ...
                if((is_array($responseParsed) && count($responseParsed)) &&
                    ...
                    if(!empty($responseObj->redirectUrl)) {
                        $responseObj->status = false;
                        $responseObj->message = __('Redirect URL couldn\t not be generated', 'mkd_twitter_feed');
                    } else {
                    ...
                    $responseObj->status = false;
                    $responseObj->message = __('Couldn\'t connect with Twitter API', 'mkd_twitter_feed');
                }
            }
        }
        ...
        if(!empty($authorizeVerifier) && !empty($authorizeToken)) {
            ...
            if(is_wp_error($response)) {
                ...
                if(is_array($responseParsed) && count($responseParsed)
                    ...
                    $responseObj->status = true;
                    $responseObj->message = __('Access token obtained', 'mkd_twitter_feed');
                }
            }
        } else {
            $responseObj->status = false;
            $responseObj->message = __('Authorize token and it\'s secret were not obtainer', 'mkd_twitter_feed');
        }
    ...
    public function fetchTweets($userId = '', $count = '', $transient = array()) {
        ...
        if(!$this->transientExists($transient)) {
            if($userId && $accessToken) {
                ...
                if(is_wp_error($response)) {
                    ...
                    if(isset($response['response']['code']) && $response['response']['code'] == 200) {
                        ...
                        $responseObj->status = false;
                        $responseObj->message = __('Couldn\'t connect with Twitter', 'mkd_twitter_feed');
                    }
                }
            } else {
                $responseObj->status = false;
                $responseObj->message = __('It seams like you haven\t connected with your Twitter account', 'mkd_twitter_feed');
            }
        } else {
            ...
            if(!empty($transientContent)) {
                ...
                $responseObj->status = false;
                $responseObj->message = __('Couldn\'t retreive content from database', 'mkd_twitter_feed');
            }
        }
